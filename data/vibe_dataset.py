...
import joblib
#from .. import encoder_modules.Encoder as *

class VIBEDataset(data.Dataset):
    def __init__(self, opt):
        super(VIBEDataset, self).__init__()
        self.opt = opt
        self.root = osp.join(self.opt.ann_dataroot, self.opt.datamode, "VIBE")
        print(self.root)
        self.data_list = []
    ...
    def __getitem__(self, index):
        ...
        print("VIBE File Name", vibe_fname.split("/")[5])
...
def main():
    v = VIBEDataset(None, "/data_hdd/fw_gan_vvt/train")
    ...
    for i_batch, sample_batched in enumerate(dataloader):
        ...
        print("Frame IDs:", frame_ids.size())
        print("Mesh Vertices:", mesh_verts.size())
        ...
        #print(frame_ids)
        for frame in range(pose_params.shape[1]):
            print(frame)
